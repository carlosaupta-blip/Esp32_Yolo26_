...
static esp_err_t threshold_post_handler(httpd_req_t *req) {
    ...
    if (received <= 0) {
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Error al recibir datos de red");
        return ESP_FAIL;
    }
    buf[received] = '\0';
    ...
    // Responder con éxito al navegador [cite: 435]
...
esp_err_t web_server_start(void) {
    ...
    uri.uri = "/api/stream";  uri.method = HTTP_GET;  uri.handler = mjpeg_stream_handler;    httpd_register_uri_handler(server, &uri);
...
/* ---------------------------------------------------------------------
 * Obtener el handle del servidor (para registrar más URIs desde main)
 * ------------------------------------------------------------------- */
